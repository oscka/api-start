...
public class ProductKafkaConsumer {
    ...
    Consumer<Message<String>> productUpdate() {
        return ((input) ->{
        try {
            log.info("[Kafka-Consumer] >> ProductKafkaConsumer.productUpdate >> Header : {}", input.getHeaders());
            log.info("[Kafka-Consumer] >> ProductKafkaConsumer.productUpdate >> Payload : {}", input.getPayload());
